...
def coarsening(data, coarsening_ratio, coarsening_method, num=None):
    # G = gsp.graphs.Graph(W=to_dense_adj(data.edge_index)[0])
    # adj = sparse_coo_tensor(data.edge_index, torch.ones(data.edge_index.shape[1]).cuda(), (data.x.shape[0], data.x.shape[0]))
    # data.edge_index = to_undirected(data.edge_index)
    adj = coo_matrix((torch.ones(data.edge_index.shape[1]), (data.edge_index[0].cpu(), data.edge_index[1].cpu())), shape=(data.x.shape[0], data.x.shape[0]))
    adj = adj.tocsr()
    G = gsp.graphs.Graph(W=adj)
    ...
    while number < len(candidate):
        ...
        if len(H.info['orig_idx']) > 10:
            C, Gc, Call, Gall = coarsen(H, r=coarsening_ratio, method=coarsening_method, num=num)
            C_list.append(C)
            Gc_list.append(Gc)
        number += 1
