...
class GCN(nn.Module):
    ...
    def fit_with_val(self, features, adj, labels, syn_mask, data, train_iters=200, initialize=True, verbose=False, normalize=True, patience=None, noval=False, **kwargs):
        ...
        if type(adj) is not list and len(labels.shape) > 1:
        ...
        if noval:
            self._train_with_val(labels, data, train_iters, verbose, syn_mask, adj_val=True)
        else:
            self._train_with_val(labels, data, train_iters, verbose, syn_mask)

    def _train_with_val(self, labels, data, train_iters, verbose, syn_mask, adj_val=False):
        if adj_val:
            feat_full, adj_full = data.feat_val, data.adj_val
        else:
            feat_full, adj_full = data.x, data.edge_index
        ...
        adj_full_norm = torch.sparse_coo_tensor(data.edge_index,
                        torch.ones((data.edge_index.shape[1])).cuda(), (data.num_nodes, data.num_nodes))
        # adj_full_norm = utils.normalize_adj_tensor(adj_full_norm, sparse=True)
        ...
        for i in range(train_iters):
            ...
            for j in range(len(labels)):
                self.train()
                optimizer.zero_grad()
                output = self.forward(self.features[j], self.adj_norm[j])
                # output = self.forward(feat_full, adj_full_norm)
                loss_train = self.loss(output[syn_mask[j]], labels[j][syn_mask[j]])
                # loss_train = self.loss(output[data.train_mask], data.y[data.train_mask])
